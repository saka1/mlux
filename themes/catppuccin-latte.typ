...
#set text(font: "Noto Sans JP", size: 12pt * scale, fill: rgb("#4c4f69"),
...
#show raw.where(block: true): it => block(
  fill: rgb("#e6e9ef"), inset: 12pt * scale, radius: 6pt * scale, width: 100%,
  text(font: ("DejaVu Sans Mono", "Noto Sans JP"), size: 10pt * scale, it))

// Inline code (raw's ShowSet applies size: 0.8em, so divide by 0.8 to compensate)
#show raw.where(block: false): it => box(
  fill: rgb("#e6e9ef"), inset: (x: 0.3em / 0.8), outset: (y: 0.15em / 0.8), radius: 3pt * scale,
...
#show quote.where(block: true): it => block(
  ...
  stroke: (left: 3pt * scale + rgb("#1e66f5")),
  text(fill: rgb("#6c6f85"), it.body))

// Tables
#set table(stroke: 0.5pt * scale + rgb("#acb0be"), inset: 8pt * scale,
...
#show strike: set strike(stroke: 1pt * scale + rgb("#5c5f77"))
...
#show line: set line(stroke: 1pt * scale + rgb("#acb0be"))

// Image placeholder: Surface 2 border
#let image-placeholder(path) = block(stroke: 0.5pt * scale + rgb("#acb0be"), inset: 8pt * scale, radius: 4pt * scale)[Image: #path]
