...
を参照。各 variant の試行ログ (ExpDecay → DampedSpring 廃案 → Kinetic 採用)
は `../2026-04-29-experiments-scroll-animation.md` に集約されている。
...
HCI の方向と逆になるため `2026-04-29-experiments-scroll-animation.md (Phase 3)` で削除。
...
「微小ピクセルを刻む」体感の主因だった (`2026-04-29-experiments-scroll-animation.md (Phase 3)`)。
...
**注 1**: half_life=40ms の根拠は `../2026-04-29-experiments-scroll-animation.md`
Phase 1。経験的な「ふわっと感じるが間延びしない」値。
...
- **DampedSpring** (`2026-04-29-experiments-scroll-animation.md (Phase 2)`):
  critical damping spring。tail が構造的に slow で「微小ピクセル creep」が
  解消できなかった (`2026-04-29-experiments-scroll-animation.md (Phase 3)`)。`Kinetic`
  に置換し削除。
...
- **Velocity clamping / position-dependent damping** など ad-hoc な工夫:
  ...
  (`2026-04-29-experiments-scroll-animation.md (Phase 2)` §未解決)。
