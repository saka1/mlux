...
1. **`a=t` (小文字) 必須**: `a=T` で送信した画像は `a=p` で参照不可（ENOENT）。送信は `a=t`（キャッシュのみ、表示しない）で行い、表示はすべて `a=p` で。
2. **`\x1b[2J` 禁止**: 画像データごと消える。配置の削除は `a=d,d=i,i=ID,q=1`（小文字 `i` = データ保持）を使う。
...
現在の方式では、画像全体の送信完了まで何も表示されない。
改良案:
- 画像を水平ストリップに分割し、上部から順次送信
- ビューポート内の部分を最優先で送信
- 残りをバックグラウンドで送信
- ただし、画像IDが複数になるためスクロール時の管理が複雑化する

### 非常に長いドキュメント

`height: auto` で数万pt の画像が生成される場合:
- PNGのサイズが巨大になる（メモリ・送信時間）
- ターミナルのストレージクォータ（`ENOSPC`）に抵触する可能性
- 対策: 一定高さを超えたら分割方式に切り替えるフォールバック
