...
\x1b_Ga=T,f=100,i=1,...\x1b\   ← 送信+表示OK
\x1b_Ga=p,i=1,...\x1b\          ← 初回配置OK（キャッシュあり）
...
### `a=T` / `a=t` どちらもキャッシュされる

当初 `a=T`（送信+表示）ではキャッシュされないと疑ったが、再検証の結果 **`a=T` でも `i=` 付きで送信すれば `a=p` で参照可能** であることを確認。ENOENT の原因は `a=T` ではなく `\x1b[2J` による画像データ破棄だった。

- `a=T`: 送信+即座表示。キャッシュされる。初回表示が不要な場合は無駄な描画が発生する
- `a=t`: 送信のみ。キャッシュされる。表示は `a=p` で行う

どちらを使っても動作するが、ビューポートクロップを使う場合は `a=t` + `a=p` の方が初回の無駄な全体表示を避けられる。

### 検証済みスクロール手順

```
1. a=T または a=t で PNG をチャンク送信（1回のみ）
2. a=p で初回ビューポート配置
3. スクロール時:
