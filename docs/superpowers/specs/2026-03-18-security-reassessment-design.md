...
- ABI フォールバック: `Ruleset::default()` (自動 ABI 交渉) は使わず、`ABI::V4` を明示指定した上で
  `AccessFs::from_all(abi)` / `AccessNet::from_all(abi)` を使う。V4 未対応カーネルでは
  `AccessNet::from_all()` が空の `BitFlags` を返すため、ネットワークルールは自動的にスキップされる。
  FS ルールは V3 相当にフォールバックする。
