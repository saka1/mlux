...
- `ABI::V3` → `ABI::V4`
- `Ruleset::default().handle_access(AccessFs)` に `.handle_access(AccessNet::from_all(abi))` を追加
...
| 5.13 - 6.1 | V1 フォールバック | 部分的 | なし |
| < 5.13 | なし | なし | なし |

Landlock クレートの best-effort モードにより自動フォールバック。

## 変更 2: パイプライン分割（fetch / compile 分離）

### 概要

`build_tiled_document()` を 2 段階に分割し、リソースフェッチとコンパイルの間に
Landlock 適用ポイントを挿入可能にする。
...
残る unsafe: `fork()` と `libc::_exit()` の 2 箇所。本質的に unsafe であり削減不可。

## 変更 4: セキュリティ設計ドキュメント

本ドキュメント (`docs/2026-03-18-design-security-reassessment.md`) を新規作成する。
...
`read`, `write` (pipe IPC), `mmap`, `munmap`, `brk`, `clock_gettime`, `close`,
`sigaltstack`, `rt_sigaction`, `exit_group` のみ許可。それ以外は KILL。
