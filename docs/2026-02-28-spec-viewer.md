...
`t` で入る。Markdown ソースから ATX 見出し (`# heading`) を抽出し、一覧表示する。

### 画面レイアウト

```
Row 0:     Table of Contents:    ← ヘッダ
Row 1..N:  見出しリスト           ← レベルに応じたインデント + • ビュレット
Last row:  ステータスバー         ← 見出し数 + キーヒント
```
...
| 対象 | ATX 見出し (`#{1,6} `) |
| コードブロック | ` ``` ` / `~~~` フェンス内の `#` は無視 |
| visual line マッピング | `md_line_range` で対応する visual line を検索、見つからない行はスキップ |
