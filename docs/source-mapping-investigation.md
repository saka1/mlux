...
**結論**: Glyph::span → Source::range() → markup.rs SourceMap のチェーンで、
...
- **現状**: `markup.rs` は `Parser::new_ext()` のみ使用し、オフセット情報を捨てている（L26）

### 段階 2: pulldown-cmark イベント → Typst マークアップ

```
(Event, Range<usize>) ストリーム
  ↓ markup.rs: markdown_to_typst()
Typst マークアップ文字列 (content_text)
```

- `markup.rs` はイベントを順次処理し、Typst マークアップ文字列を構築する
...
ただし、markup.rs の SourceMap が行単位の粒度を持つ必要がある。
...
テーブルは markup.rs で `#table(columns: N, ...)` に変換される。
...
リスト全体が1つの Group。markup.rs では各項目が `- ` or `+ ` で始まる行として出力。
...
markup.rs で `#quote(block: true)[...]` に変換。
全体が1つの BlockMapping。
...
[prefix_len..prefix_len+content_len]  ← content_text (markup.rs の出力)
...
markup.rs ではテーブルは `End(Table)` 時にまとめて emit される（L198-206）。
