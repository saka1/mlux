...
信頼する側 (supervisor)          │  信頼しない側 (renderer)
  ...
  設定読み込み                    │  convert.rs (Typst マークアップ生成)
...
### Phase 2: fork + Landlock サンドボックス（Linux）

`fork()` で子プロセスを生成し、子プロセス内で render パイプライン全体を
Landlock サンドボックス下で実行する。
...
親プロセス:
  ...
  fork() ──→ 子プロセス:
  │            Landlock restrict_self()  ← read: git root, write: 出力dir
  │            ─────────────────────────
  ...
  │            Typst compile
  │            タイル分割
  │            RPC サーバー (メタデータ + PNG 提供)
  │
  ├── RPC: メタデータ受信 (タイル数, visual lines, 見出し, URL)
  ├── RPC: render_tile(i) → PNG bytes (オンデマンド)
  │
  ターミナル I/O / ファイル監視 / 設定リロード
```

**fork() の利点:**
- フォント、markdown バイト列が COW ページで子に引き継がれる → シリアライズ不要
- 子は fork 直後に `restrict_self()` するだけ → IPC は結果側のみ
- バイナリ分割不要。同一バイナリで supervisor / renderer の役割を分ける
- 画像読み込みは子が Landlock scope 内で自力で行う → 事前読み込み不要

**RPC 境界:**

`TiledDocument` の現在の公開 API がそのまま RPC インターフェースになる:
- `tile_count()`, `visual_lines()`, `headings()` → 初回コンパイル後にメタデータ送信
- `render_tile(i) → Vec<u8>` → オンデマンド要求/応答

`RemoteTiledDocument` を同じインターフェースで実装すれば、
viewer 側のコード変更は最小限で済む。

**viewer モードでの再コンパイル:**

ファイル変更時は子プロセスを再 fork する。親は新しい markdown バイト列を
読み込み、fork し、旧子プロセスを終了する。FontCache は親に残るため
フォントスキャンは初回のみ。
...
| 条件 | 読み取り許可 | 書き込み許可 |
|------|-------------|-------------|
| render モード | git root or 入力親ディレクトリ | 出力ディレクトリ |
| viewer モード | git root or 入力親ディレクトリ | なし（PNG は RPC で親に返す） |

viewer モードでは子に書き込み権限が一切不要。render モードより厳しい制約になる。
...
**render モード Landlock**（Phase 3 代替案）は実装済み。`src/sandbox.rs` で
Landlock を適用し、Typst コンパイル前にファイルアクセスを制限する。
入力ファイルが git リポジトリ内の場合は git root 以下を読み取り許可、
それ以外は入力ファイルの親ディレクトリのみ。書き込みは出力ディレクトリのみ。
`--no-sandbox` フラグで無効化可能。非対応環境では graceful degradation。
