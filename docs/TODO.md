...
### D-1b. `md_line_exact` の汎用化（リスト等の行単位ヤンク） ✓

- `resolve_md_line_range` の改行カウント方式をコードブロック以外にも拡張
- Typst/MD 間の改行数一致チェックで安全性を保証（テーブル・ネスト引用は自動フォールバック）
- 対応ブロック: リスト (UL/OL), 段落, 見出し
- 非対応ブロック: テーブル, 引用（改行数不一致のため現行動作を維持）
- 設計ドキュメント: `docs/line-exact-generalization.md`
- 統合テスト4件追加
